/* Note: This file should be deleted after moving to jquery UI 1.8
 ...
 *
 * Revision: $Id: jquery.autocomplete.js,v 1.1 2012/05/04 02:37:19 ssprouse Exp $
 *
 * Update 7/27/2011 aroy: Altered the Cache.load function to return an empty array instead of null when it finds
 *   an empty cache record. Altered the request function to bypass cache only if it returns null (e.g. if it returns
 *   empty array then treat that as a valid cache get). These changes effectively make it so when matchSubset is true,
 *   ajax calls will not be issued for strings where prefixes have had 0 results: e.g. Searching for "1234" will not
 *   fire off an ajax call if "123" was previously cached with 0 results.
 * Update 8/4/2011 aroy: Add extraParamsRequired that prevents ajax request if an extraParam function returns null
 *
 */
;
(function ($) {
    $.fn.extend({autocomplete2:function (urlOrData, options) {
                ...
                try {
                    $.each(options.extraParams, function (key, param) {
                        ...
                        if (typeof param === "function") {
                            keyVal = param();
                            if (options.extraParamsRequired === true & keyVal === null) {
                                abort = true;
                            }
                        } else {
                            keyVal = param;
                        }
                        extraParams[key] = keyVal;
                    });
    ...
    $.Autocompleter.defaults = {inputClass:"ac_input", resultsClass:"ac_results", loadingClass:"ac_loading", minChars:1, delay:400, matchCase:false, matchSubset:true, matchContains:false, cacheLength:10, max:100, mustMatch:false, extraParams:{}, extraParamsRequired:false, selectFirst:true, formatItem:function (row) {
        return row[0];
