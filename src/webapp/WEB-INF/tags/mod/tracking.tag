...
<script language="javascript" type="text/javascript">
    var fc_content='${requestScope.context.hostName}/${empty(state)
        ? requestScope.context.stateOrDefault.abbreviationLowerCase : state}/${empty(pageType)
        ? "-" : pageType}/${pageName}/${empty(id)
        ? "-" : id}/${empty(schoolType)
        ? "-" : schoolType}/${empty(schoolLevel)
        ? "-" : schoolLevel}/${empty(locale)
        ? "-" : locale}/${requestScope.context.hostName == 'www.greatschools.net'
        ? "0" : "1"}/${requestScope.context.stateOrDefault.subscriptionState ? "1" : "0"}';
