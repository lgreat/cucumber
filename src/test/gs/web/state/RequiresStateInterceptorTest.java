 ...
 * $Id: RequiresStateInterceptorTest.java,v 1.8 2009/12/04 19:37:26 npatury Exp $
 */
...
public class RequiresStateInterceptorTest extends BaseControllerTestCase {
    ...
    public void testRequiresStateInterceptorNoState() throws Exception {
        ...
        assertEquals("http://www.greatschools.org/selectAState.page?prompt=Please+select+a+state+to+continue.&url=%2Fpath%2FmySchool.page",
    ...
    public void testRequiresStateInterceptorNoStateButOtherParam() throws Exception {
        ...
        boolean b = _interceptor.preHandle(getRequest(), getResponse(), null);
        assertEquals("http://www.greatschools.org/selectAState.page?prompt=Please+select+a+state+to+continue.&url=%2Fpath%2FmySchool.page%3Fcity%3DLincoln",
                getResponse().getRedirectedUrl());
        assertFalse(b);
    }
    ...
    public void testRequiresStateInterceptorNoStateButTwoOtherParam() throws Exception {
        ...
        boolean b = _interceptor.preHandle(getRequest(), getResponse(), null);
        assertEquals("http://www.greatschools.org/selectAState.page?prompt=Please+select+a+state+to+continue.&url=%2Fpath%2FmySchool.page%3Fcity%3DLincoln%26amp%3Bzip%3D12345",
                getResponse().getRedirectedUrl());
        assertFalse(b);
    }
    ...
    public void testRequiresStateInterceptorNoStateWithContext() throws Exception {
        ...
        boolean b = _interceptor.preHandle(getRequest(), getResponse(), null);
// Mock object differs in behavior from Tomcat: assertEquals("http://www.greatschools.org/gs-web/selectAState.page?prompt=Please+select+a+state+to+continue.&url=/gs-web/path/mySchool.page",
        assertEquals("http://www.greatschools.org/gs-web/selectAState.page?prompt=Please+select+a+state+to+continue.&url=%2Fgs-web%2Fpath%2FmySchool.page",
                getResponse().getRedirectedUrl());
        assertFalse(b);
    }
